...
export default function TopOrphan() {
  ...
  let cloudResourceTypes = findOcc(initialData, "ResourceType");
  ...
  initialData.forEach((item) => {
    const { ResourceType, CostSaving } = item;
    const convertCost = parseFloat(CostSaving);
    if (aggregatedCostSavings.hasOwnProperty(ResourceType)) {
      aggregatedCostSavings[ResourceType] += convertCost;
    } else {
      aggregatedCostSavings[ResourceType] = convertCost;
    }
  });
  ...
  return (
    <div className="d-flex flex-column  w-100">
      <small className="fw-bold nav-font">Top 10 Orphaned</small>
