...
import { DialogActions, Button, Breadcrumbs } from "@mui/material";
...
const CostCodeOnboarding = () => {
  ...
  const [subDropdownsObject2, setSubDropDownsObject2] = useState<any>([]);
  ...
  const [formData, setFormData] = useState<any>([
    {
      ...
      Title: "CostCode Name",
      value: "",
      description: "Please enter CostCode Name"
    },
    {
      inputType: "text",
      ApiKey: "CostCode",
      Title: "CostCode",
      value: "",
      description: "Please enter CostCode"
    },
    {
    ...
    },
    {
      ...
      Title: "Approvers",
      value: [],
      description: "Please select CostCode Approvers"
    }
  ]);
  ...
  const crumbs = !id
    ? { to: "/costcode-onboarding", title: "CostCode Onboarding" }
    : { to: pathname, title: "CostCode update" };
  ...
  const breadcrumbs = [
    ...
    <Link
      className="border-bottom border-primary fw-normal"
      to="/onboarding/costcode-onboarding-details"
      title="costcode"
    >
      Costcode
    </Link>,
  ...
  useEffect(() => {
    const fetchData = async () => {
      const apiEndPoints = ['users', 'BU', 'mastersubscriptions', 'Application'];
  ...
  const dropdownsObject = { "Approvers": Object.keys(filterData("FullName", data["users"])), "Applications": Object.keys(filterData("AppName", data["Application"])), "BusinessUnit": Object.keys(filterData("BUName", data["BU"])), "Subscriptions": Object.keys(filterData("SubscriptionID", data["mastersubscriptions"])) }
console.log("Application", data["users"])
...
const hum = (newValue) => {
  ...
  if (dataa) {
    ...
    for (var i = 0; i < exist.length; i++) {
      ...
      app.push(...exist[i].Applications)
      }
    console.log("exist",dataa, exist,newValue,memb,sub,app)
    setDropDownsObject1(memb)
    setSubDropDownsObject2(sub)
    setAppDropdownsObject2(app)


  }
  console.log("dropdownsObject1", dataa)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Api.getDataByID(testapi.costCode, id);
  ...
  const handleFormSave = async () => {
    ...
    if (isFormValid()) {
      try {
        const response = !id ? Api.postData(testapi.costCode, postBody) : Api.putData(testapi.costCode, postBody, id)
        response.then(data => {
        ...
        setShowValidation(false);
        setShowModal(true);
        setModalMessage("CostCode Onboarded Successfully!!");
      } catch (error) {
        setShowModal(true);
      ...
      <div className="p-2 onboarding-page-h bg_color">
        <div className="d-flex justify-content-between align-items-center">
          <div className="d-flex align-items-center ps-2">
            <h4 className="p-2 ps-3 fw-bold k8title"> CostCode Onboarding </h4>
          ...
          <div className="launchStack ps-5 ms-5">
            <form className="row gy-2 align-items-center">
              {formData.map((curr: any, index: number) => {
                { { console.log("dropdownsObject1", index, curr.ApiKey) } }
                ...
                return (
                  <div className={`col-md-6 d-flex justify-content-start`}>
                    ...
                    ) : curr.inputType == "MultiMenus" ? (<label className="form-label label-width w-75">
...
export default CostCodeOnboarding;