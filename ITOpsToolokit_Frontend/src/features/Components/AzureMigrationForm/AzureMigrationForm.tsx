...
const AzureMigrationForm = () => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://ec2-16-52-107-209.ca-central-1.compute.amazonaws.com:8006/azure_migration_assessment",
        payload
      );
  ...
  const handleExcelUpload = async () => {
    ...
    try {
      ...
      setApiResponse(null);
      const response = await axios.post(
        "http://ec2-16-52-107-209.ca-central-1.compute.amazonaws.com:8006/azure_migration_assessment/upload-excel/",
  ...
  return (
    ...
    >
      ...
      >
        Azure Migration Assessment Bot
      </h1>
      <div className="row">
        <div
          className="col-6 chatbot-container "
        ...
        >
          ...
          >
            <button
              className={`Gentab ${activeTab === "tab1" ? "active" : ""}`}
              onClick={() => setActiveTab("tab1")}
            >
              Form Input
            </button>
            <button
              className={`Gentab ${activeTab === "tab2" ? "active" : ""}`}
          ...
          <div
            className="  tab_content-wrapper"
            style={{ paddingLeft: "5rem", height: "29rem" }}
          >
            <div
              className={`tab_content ${activeTab === "tab1" ? "active" : ""}`}
              id="tab1"
            >
              <form
                ...
                style={{
                  ...
                  gap: "14px",
                  maxWidth: 800, // increased to allow side-by-side layout
                  marginTop: 20,
                }}
              >
                {/* Project & Client in a row */}
                ...
                {/* Dynamic fields in 2-column layout */}
                <div style={{ display: "flex", flexWrap: "wrap", gap: "14px" }}>
                  {fields.map((fieldName) => (
                    <div style={{ flex: "1 1 48%" }} key={fieldName}>
                      <Controller
                        name={fieldName}
                        control={control}
            ...
            <div
              className={`tab_content ${activeTab === "tab2" ? "active" : ""}`}
              id="tab2"
            >
              <Box sx={{ mt: 4 }}>
                <input
...
export default AzureMigrationForm;
