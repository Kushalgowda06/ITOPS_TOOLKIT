...
class ChangeManagement:
    def __init__(self):
        ...
        conf_f_path = ROOT_PATH + '/mim_project/config/mim_conf.json'
        ...
        # Initiate vault
        vault_path = ROOT_PATH + '/mim_project/config/.vault_token'
    ...
    def create_change(self, cr_create_prompt):
        refined_prompt = cr_create_prompt + "\n\nWith this background, create a proper JSON Object for change request with keys - \n"
        refined_prompt += "short_description\ndescription\nimplementation_plan\nbackout_plan\ntest_plan\nchg_model: normal or emergency"
        refined_prompt += "\nPlease ensure that the value corresponding to each key is a string. In a string, new point should start on new line. Examples can have different IPs/names. But document should not influence of those specifics. Please feel free to add missing steps and elaboration. Every plan should be detailed including CLI/GUI execution steps, configuration samples etc. It should always exceed 20 lines."
    ...
    def analyze_change_impact_with_service_map(self, short_description, description, ci_items):
        ...
        if service_map_explanations != []:
            ...
            prompt += "\n\nImpact should be in terms of any service disruption that can happen when change activity is ongoing on overall architecture. The impact analysis shouldn't just focus on 1 component, rather it's consequences on other connected components too. If applicable, also include challenges that end user can face while accessing the application or services. Explain any disruption or unavailability through logical example. It is must to justify why and how the impact happens. Also, ensure to include/append risk mitigation plan in the context of impact rather than being generic."
