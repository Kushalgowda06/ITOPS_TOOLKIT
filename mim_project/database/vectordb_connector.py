...
class VectorDatabase:
    def __init__(self):
        ...
        self.similarity_threshould = mim_conf['similarity_threshould']
    ...
    def update_data(self, collection = "", update_dict = {}, conditions = []):
        
        if update_dict == {}:
            return 0 
        
        conditions.append(
            {"col": "metadata->>'number'", "op": "eq", "val": update_dict["kb_number"]}
        )
        
        (conditional_query, params) = self.handle_conditions(conditions)

        update_data = {}
        update_data['embedding'] = self.em_model.embed_query(update_dict['content'])
        update_data['content'] = update_dict['content']
        ...
        try:
            ...
            raise CustomError('Error updating data in database - ' + str(e))
